...
[NOTE]
====
* À compter de novembre 2023, vous ne pourrez plus lancer de mise à niveau de composant à l'aide de NetApp Hybrid Cloud Control ou de l'API REST, car les certificats de clé de signature (privée et publique) ont expiré le 5 novembre 2023. Vous pouvez contourner ce problème enlink:task_hcc_upgrade_management_node.html["mise à niveau du nœud de gestion sans déplacement des données"] à la version 12.8 ou ultérieure avant d'utiliser NetApp Hybrid Cloud Control ou l'API REST pour mettre à niveau les services de gestion, le logiciel Element et le firmware de stockage.
+
Si vous déployez un nouveau nœud de gestion avec la version 12.8 ou ultérieure, vous pouvez utiliser NetApp Hybrid Cloud Control ou l'API REST pour effectuer les mises à niveau des composants dans l'ordre indiqué.<<sys_upgrade,séquence de mise à niveau du système>> .
...
[IMPORTANT]
====
** Le pack de services de gestion 2.27 inclut le plug-in Element pour vCenter Server 5.5, qui est uniquement compatible avec le nœud de gestion 12.8 ou ultérieur. Lorsque vous effectuez une mise à jour vers les services de gestion 2.27, vous devez modifier la séquence de mise à niveau et mettre à jour le bundle de services de gestion _après_ la mise à niveau vers Element 12.9 pour assurer la compatibilité entre le nœud de gestion et les services de gestion.
** Si vous effectuez une mise à jour vers les services de gestion 2.21.61 à 2.26.40, vous devez mettre à jour le bundle de services de gestion _avant_ la mise à niveau vers Element 12.9.
