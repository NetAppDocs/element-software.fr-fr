...
[NOTE]
====
* Vous ne pouvez pas utiliser cette méthode pour désactiver le chiffrement logiciel au repos. Pour désactiver le chiffrement logiciel au repos, vous devez link:reference_element_api_createcluster.html["créez un nouveau cluster"] le chiffrement logiciel des données au repos est désactivé.
* Pour consulter l'état actuel du chiffrement au repos, du chiffrement logiciel au repos ou des deux sur le cluster, utilisez link:../api/reference_element_api_getclusterinfo.html["obtenir la méthode d'information sur le cluster"]. Vous pouvez utiliser le `GetSoftwareEncryptionAtRestInfo` link:../api/reference_element_api_getsoftwareencryptionatrestinfo.html["méthode d'obtention des informations que le cluster utilise pour chiffrer les données au repos"].
