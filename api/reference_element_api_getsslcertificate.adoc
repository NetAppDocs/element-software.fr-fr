...
summary: Vous pouvez utiliser la méthode GetSSLCertificate pour récupérer le certificat SSL actuellement actif sur les nœuds de stockage du cluster. 
---
= GetSSLCertificate
:allow-uri-read: 
:icons: font
...
[role="lead"]
Vous pouvez utiliser le `GetSSLCertificate` Méthode pour récupérer le certificat SSL actuellement actif sur les nœuds de stockage du cluster.
...
certificat
 a| 
Texte intégral encodé au PEM du certificat.
 a| 
chaîne
...
détails
 a| 
Les informations décodées du certificat.
 a| 
Objet JSON
