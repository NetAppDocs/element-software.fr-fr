...
Si vous devez ajouter manuellement une nouvelle post-installation de ressource, utilisez le nouveau compte utilisateur HCC que vous avez créé dans  up a new user account in vCenter. Pour plus d'informations, voir link:task_mnode_add_assets.html["Ajoutez une ressource de contrôleur au nœud de gestion"].
