...
NOTE: Le chiffrement logiciel au repos est activé par défaut dans tous les nœuds SolidFire Enterprise SDS. Le chiffrement logiciel au repos fournit une couche logicielle principale de chiffrement de sécurité des données dans les nœuds SDS d'entreprise SolidFire, qui n'incluent pas les disques à autocryptage (SED).
