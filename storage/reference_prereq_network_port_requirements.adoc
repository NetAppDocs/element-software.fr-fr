...
[role="lead"]
Vous devrez peut-être autoriser les ports TCP et UDP suivants via le pare-feu de votre centre de données pour pouvoir gérer le système à distance et permettre aux clients situés à l'extérieur de votre centre de données de se connecter aux ressources. Certains de ces ports peuvent ne pas être requis, selon l'utilisation du système.
